...
def extract_geometry_points(geometries, meshing_parameters=None):
    ...
    Args:
        ...
        meshing_parameters: Optional Rhino MeshingParameters. This will only be used if
    ...
    for fid in xrange(brep.Faces.Count):
        ...
        if not brep.Faces[fid].IsPlanar(tol):
            ...
            if len(border) > 1:
                ...
                if not is_points_sorted_anti_clockwise(
                        points_sorted,
                        get_surface_center_ptand_normal(geometry).normal_vector):
...
def get_surface_center_ptand_normal(geometry):
...
def check_planarity(hb_surface, tolerance=1e-3):
    ...
    """
    try:
        return hb_surface.geometry.Faces[0].IsPlanar(tolerance)
